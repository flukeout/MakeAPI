...
module.exports = function( makeModel, apiUserModel, env ) {
  ...
  function devKeyLookupStrategy( publickey, callback ) {
    ...
    if ( publickey === DEVKEY ) {
      ...
      // generate a fake password on the fly
      credentials.key = require( "uuid" ).v4();
    }
  ...
  // make sure NODE_ENV === "development" and "USE_DEV_KEY_LOOKUP" == true
  if ( env.get( "NODE_ENV" ) === "development" && env.get( "USE_DEV_KEY_LOOKUP" ) ) {
    credentialsLookupStrategy = devKeyLookupStrategy;
  } else {
    credentialsLookupStrategy = databaseKeyLookupStrategy;
  }
